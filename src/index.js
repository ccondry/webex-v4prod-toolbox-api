...
const exceptions = {
  path: [{
    url: /\/api\/v1\/webex-v4prod\/version/i,
    methods: ['GET']
  }]
}
...
app.use('/api/v1/webex-v4prod/session', require('./routes/session'))
// post to start user demo provisioning
app.use('/api/v1/webex-v4prod/provision', require('./routes/provision'))
// get this software version
app.use('/api/v1/webex-v4prod/version', require('./routes/version'))

/*
Go
*/
app.listen(port, () => { 
  console.log(`webex-v4prod-toolbox-api version ${pkg.version} started listening on port ${port} in ${process.env.NODE_ENV} mode.`)
})
